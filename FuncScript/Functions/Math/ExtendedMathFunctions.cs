...
namespace FuncScript.Functions.Math
{
    ...
    public class RandomFunction : IFsFunction
    {
        ...
        public object Evaluate(object par)
        {
            ...
            if (pars.Length != 1)
                return new FsError(FsError.ERROR_PARAMETER_COUNT_MISMATCH, $"{Symbol}: Expected a single seed parameter.");

            var seed = MathFunctionHelper.RequireNumber(this, pars[0], "seed");
            if (seed.HasError)
                return seed.Error;

            var normalized = NormalizeSeed(seed.Value);
            return GenerateSeededRandom(normalized);
        }

        static string NormalizeSeed(double value)
        {
        ...
        static uint BuildSeedState(string text)
        {
        ...
        static double GenerateSeededRandom(string text)
        {
            const uint goldenGamma = 0x9e3779b9;
            uint state = unchecked(BuildSeedState(text) + goldenGamma);
            state = unchecked((state ^ (state >> 15)) * (state | 1));
            state ^= state + unchecked(((state ^ (state >> 7)) * (state | 61)));
            uint normalized = state ^ (state >> 14);
            return normalized / 4294967296d;
        }

        public string ParName(int index) => index == 0 ? "seed" : string.Empty;
    }
